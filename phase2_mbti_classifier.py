...
def bootstrap_ci_auc_multiclass(y_true, y_proba, n_bootstrap=2000, alpha=0.05):
    """Bootstrap 95% CI for macro AUC-ROC (multi-class OvR)"""
    n = len(y_true)
    ...
    for _ in range(n_bootstrap):
        ...
        try:
            ...
            s = roc_auc_score(y_bin, y_proba[idx], average="macro", multi_class="ovr")
        except ValueError:
            s = 0.5
        scores.append(s)
    return np.percentile(scores, 100*alpha/2), np.percentile(scores, 100*(1-alpha/2))
...
for dim_name, tinfo in TARGETS.items():
    ...
    if n_classes == 2:
        ...
        try:
            ...
            auc_v = np.nan
        auc_lo, auc_hi = bootstrap_ci_auc_multiclass(y_true_all, y_proba_all)
